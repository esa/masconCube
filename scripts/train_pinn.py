...
from mascon_cube.training import ValidationConfig


def train(asteroid: str, use_tensorboard: bool) -> PinnGM:
    config = PinnTrainingConfig(asteroid=asteroid)
    device = "cuda" if torch.cuda.is_available() else "cpu"

    val_dataset = torch.load(VAL_DATASETS_DIR / f"{asteroid}_1000_spherical_0_2.pt").to(
        device
    )
    val_config = ValidationConfig(val_dataset=val_dataset, val_every_n_epochs=5)
